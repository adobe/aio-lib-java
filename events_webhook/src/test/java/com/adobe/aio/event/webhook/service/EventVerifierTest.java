...
public class EventVerifierTest {
  ...
  public void setup() {
    ...
    setupEndpointStub();
    eventVerifier = new EventVerifier("http://localhost:" + ADOBEIO_CDN_PORT);
  }
  ...
  public void testVerifyValidSignature() {
    String eventPayload = getTestMessage();
    Map<String, String> httpRequestHeaders = getTestHeadersWithValidSignature();
    boolean result = eventVerifier.verify(eventPayload, API_KEY, httpRequestHeaders);
  ...
  public void testVerifyInvalidSignature() {
    ...
    boolean result = eventVerifier.verify(message, API_KEY, headers);
  ...
  @Test
  public void testVerifyInvalidApiKey() {
    String message = getTestMessage();
    Map<String, String> headers = getTestHeadersWithValidSignature();
    boolean result = eventVerifier.verify(message, ANOTHER_API_KEY, headers);
    Assert.assertFalse(result);
  }
