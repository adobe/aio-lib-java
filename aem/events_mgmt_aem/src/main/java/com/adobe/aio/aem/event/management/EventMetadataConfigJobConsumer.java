...
@Component(service = JobConsumer.class, immediate = true, property = {
    JobConsumer.PROPERTY_TOPICS + "=" + EventMetadataConfigJobConsumer.AIO_CONFIG_EVENT_METADA_JOB_TOPIC,
    "label = Adobe I/O Events Metadata Config Job Consumer",
    "description =Adobe I/O Events Metadata Config Job Consumer"
})
public class EventMetadataConfigJobConsumer implements JobConsumer {
  ...
  public JobResult process(final Job job) {
    if (job.getProperty(AIO_EVENT_CODE_PROPERTY)!=null) {
      ...
      try {
        ...
        log.info("Adobe I/O Events Metadata Config Job Consumer completed:"
            + " registered event metadata `{}`", registeredEventMetadata);
        return JobResult.OK;
      } catch (Exception e) {
        log.error("Adobe I/O Events Metadata Config Job Consumer `{}` processing failed: `{}", job,
            e.getMessage(), e);
        eventMetadataStatusSupplier.addStatus(eventCode,
      ...
      log.error("Adobe I/O Events Metadata Config Job Consumer `{}` is missing a `{}", job,
